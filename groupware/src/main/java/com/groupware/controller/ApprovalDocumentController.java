...
public class ApprovalDocumentController {
    ...
    // 결재선 모달
    @GetMapping(path = {"/approval-write-modal"})
    public String approvalWriteModal(Model model, HttpSession session) {
        EmployeeDto loginUser = (EmployeeDto) session.getAttribute("loginUser");
        int empId = loginUser.getEmpId();
        ...
        return "/pages/approval/approval-write-modal";
    }
    ...
    public String approvalWriteSave(ApprovalDto approval,
        ...
        //기존 데이터를 날리고 다시 insert
//        approvalService.writeReApproval(approval);
    ...
    public String approvalBeforeDetail(ApprovalDto approvalDocumentNo, Model model , int pageNo) {
