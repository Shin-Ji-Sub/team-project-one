...
public class MailController {
    ...
    public String write(@ModelAttribute("mail") MailDto mail,
        ...
        for (String recipient : emailAddresses) {
            // 이메일 주소 유효성 검사
            boolean isValidEmail = mailService.isEmailRegistered(recipient); // 이메일 등록 여부 확인
            if (!isValidEmail) {
                attributes.addFlashAttribute("alertMessage", "전송에 실패했습니다. 이메일 주소를 확인해 주세요.");
                return "redirect:/mailbox/write"; // 작성 화면으로 돌아가기
            }
